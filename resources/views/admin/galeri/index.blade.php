      ...
      <div class="bg-white p-6 shadow sm:rounded-lg">
        <section>
          <header class="mb-6 flex items-center justify-between">
            <h2 class="text-lg font-medium text-gray-900">Admin Galeri</h2>
            ...
            <table class="min-w-full divide-y divide-gray-200 rounded-lg">
              ...
              <tbody class="divide-y divide-gray-200 text-sm">
                @forelse ($galeris as $galeri)
                  <tr>
                    <td class="px-4 py-2">{{ $galeri->id }}</td>
                    <td class="px-4 py-2 font-semibold text-gray-900">{{ $galeri->judul }}</td>
                    <td class="px-4 py-2">{{ ucfirst($galeri->kategori) ?? '-' }}</td>
                    <td class="px-4 py-2">{{ $galeri->tanggal ? \Carbon\Carbon::parse($galeri->tanggal)->format('d-m-Y') : '-' }}</td>
                    <td class="px-4 py-2">
                      @if ($galeri->foto)
                        <img src="{{ asset('storage/' . $galeri->foto) }}" alt="{{ $galeri->judul }}" class="w-16 h-16 object-cover rounded">
                      @else
                        Tidak Ada Foto
                      @endif
                    </td>
                ...
                @empty
                  <tr>
                    <td colspan="6" class="text-center px-4 py-3 text-gray-500">Belum ada data galeri</td>
                  </tr>
                @endforelse
              </tbody>
            </table>
