...
class LombaUserController extends Controller
{
    ...
    {
        $allowed = ['mapsi', 'literasi', 'fls2n', 'bahasa-jawa', 'berprestasi', 'motivasi-inspiratif'];

        abort_unless(in_array($kategori, $allowed), 404);

        $judul = ucwords(str_replace('-', ' ', $kategori));
        $data = Lomba::where('kategori', $kategori)->get();

        return view('pages.lomba.kategori', compact('data', 'kategori', 'judul'));
    }
